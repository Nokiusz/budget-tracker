...
const Add = () => {
    ...
    const { categoriesData, typesData, currenciesData, prioritiesData, BASE_URL } = useContext(GlobalContext);
                ...
                >
                    <Select placeholder="Select a currency" allowClear>
                        {currenciesData.map((currency, index) => {
                            return <Option key={index} value={currency.id}>{currency.name} ({currency.acronym})</Option>
                        })}
                    </Select>
                </Form.Item>
